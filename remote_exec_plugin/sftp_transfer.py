...
def _ssh_exec(server_ip, username, password, command, ssh_port=22, timeout=120):
...
def upload_file(local_path, server_ip, username, password, remote_path, ssh_port=22):
    ...
    code, _, err = _ssh_exec(server_ip, username, password, mkdir_cmd, ssh_port=ssh_port)
    if code != 0:
...
def download_file(server_ip, username, password, remote_file, local_path, ssh_port=22):
    _validate_host(server_ip)
...
def download_directory_as_tar(server_ip, username, password, remote_dir, local_tar_path, ssh_port=22):
    _validate_host(server_ip)
    ...
    code, _, err = _ssh_exec(server_ip, username, password, tar_cmd, ssh_port=ssh_port, timeout=600)
    if code != 0:
        raise UploadError("Remote tar command failed: {0}".format(err.strip()))

    try:
        download_file(server_ip, username, password, remote_tar, local_tar_path, ssh_port=ssh_port)
    finally:
        _ssh_exec(server_ip, username, password, "rm -f {0}".format(shell_quote(remote_tar)), ssh_port=ssh_port)
