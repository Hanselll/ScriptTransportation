...
def _is_remote_directory(server_ip, username, password, remote_path, ssh_port=22):
...
def fetch_report(server_ip, username, password, remote_report_path, local_report_key, ssh_port=22):
    logger.info("Start fetching report from %s:%s (ssh_port=%s)", server_ip, remote_report_path, ssh_port)
    ...
    try:
        if _is_remote_directory(server_ip, username, password, remote_report_path, ssh_port=ssh_port):
            local_tar = str(local_report_dir / "report_bundle.tar.gz")
            download_directory_as_tar(server_ip, username, password, remote_report_path, local_tar, ssh_port=ssh_port)
            ...
            download_file(server_ip, username, password, remote_report_path, local_file, ssh_port=ssh_port)
