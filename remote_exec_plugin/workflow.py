...
def deploy_and_run(file_key, server_ip, username, password, remote_path, run_command, ssh_port=22):
    ensure_shared_dirs()
    file_info = read_shared_file(file_key)
    local_path = resolve_shared_path(file_key)
    upload_result = upload_file(local_path, server_ip, username, password, remote_path, ssh_port=ssh_port)
    exec_result = run_remote_command(server_ip, username, password, run_command, ssh_port=ssh_port)
    return {"file": file_info, "upload": upload_result, "execution": exec_result}


def fetch_and_analyze(server_ip, username, password, remote_report_path, local_report_key, ssh_port=22):
    fetch_result = fetch_report(server_ip, username, password, remote_report_path, local_report_key, ssh_port=ssh_port)
...
def run_full_job(
    ...
    try:
        deploy_result = deploy_and_run(file_key, server_ip, username, password, remote_path, run_command, ssh_port=ssh_port)
        execution = deploy_result["execution"]
        try:
            fetched = fetch_and_analyze(
                server_ip, username, password, remote_report_path, local_report_key, ssh_port=ssh_port
            )
